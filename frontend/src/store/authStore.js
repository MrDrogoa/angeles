...
export const useAuthStore = defineStore('auth', () => {
  ...
  sessionSyncService.onLogout(() => {
    console.log('🔄 Logout detectado en otra pestaña, limpiando sesión local...')
    user.value = null
  ...
  const hasAuthCookies = () => {
    ...
    if (typeof document !== 'undefined') {
      ...
      const hasAuthCookies = cookies.includes('ACCESS_TOKEN') || 
      ...
      const hasStorageTokens = localStorage.getItem('accessToken') || localStorage.getItem('sessionId')
      
      // Verificar si hay datos de usuario persistidos
      const hasPersistedUser = localStorage.getItem('user_data')
      
      const hasAuth = hasAuthCookies || !!hasStorageTokens || !!hasPersistedUser
      console.log(`🍪 Cookies detectadas: ${hasAuthCookies ? 'Sí' : 'No'}`)
      console.log(`💾 Tokens en localStorage: ${hasStorageTokens ? 'Sí' : 'No'}`)
  ...
  const register = async (userData) => {
    try {
      ...
      const errorMessage = err.response?.data?.error || err.message || 'Error en el registro'
      setError(errorMessage)
  ...
  const logout = async () => {
    try {
      ...
      try {
        ...
        console.warn('⚠️ Error al hacer logout en backend (continuando de todos modos):', err.message)
      }
      ...
      checkAuthPromise = (async () => {
        ...
        if (persistedUserData) {
          try {
            ...
            if (age < 3600000) { // 1 hora en ms
              user.value = userData
              console.log('💾 Usuario restaurado desde localStorage (temporal):', userData.email)
            } else {
              console.log('⏰ Datos de usuario en localStorage expirados, limpiando...')
              localStorage.removeItem('user_data')
            }
          } catch (error) {
            console.warn('⚠️ Error al parsear datos de usuario desde localStorage:', error)
            localStorage.removeItem('user_data')
          }
        }
        ...
        if (!hasAuthCookies()) {
          console.log('👤 No hay cookies de sesión ni tokens - usuario no autenticado')
          clearUser()
          return
        }
        ...
        try {
          ...
          if (err.response?.status === 401) {
            console.log('🗑️ Tokens inválidos detectados (401), limpiando localStorage')
          ...
          if (hasStorageTokens && !err.response) {
            console.log('⚠️ Hay tokens almacenados pero falló la verificación por red - manteniendo estado temporalmente')
            // No limpiar el usuario inmediatamente si ya está cargado desde localStorage
            if (!user.value) {
              clearUser()
            }
          } else {
            console.log('👤 No hay sesión activa - iniciando sin autenticación')
            clearUser()
          }
        }
      })()
  ...
  const updateProfile = async (profileData) => {
    try {
      ...
      const errorMessage = err.response?.data?.error || err.message || 'Error actualizando perfil'
      setError(errorMessage)
  ...
  const changePassword = async (passwordData) => {
    try {
      ...
      const errorMessage = err.response?.data?.error || err.message || 'Error cambiando contraseña'
      setError(errorMessage)
